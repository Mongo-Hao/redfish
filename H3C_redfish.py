...
class GetHostInfo(object):
    def __init__(self,ipaddr,username,password):
        ...
        token=0
        tokenurl=self.URLprefix+'/redfish/v1/SessionService/Sessions'  ##dell获取token的ID
        print(tokenurl)
        data={
...
def Collect_Info(ipaddr,username,password):
    H3CR4900=GetHostInfo(ipaddr,username,password)
    ...
    temp_system_result1= H3CR4900.GetInfo(select_system_total)
    if isinstance(temp_system_result1,dict) and ('error' not in  temp_system_result1.keys() ):
        ...
        #print ('Processors',H3CR4900.GetInfo(cpu))
        cpu_result1 = H3CR4900.GetInfo(cpu)
        ...
        for single_cpuurl in cpu_URLsuffix_list:
            singlecpu_result2= H3CR4900.GetInfo(single_cpuurl)
        ...
        memory_result1 = H3CR4900.GetInfo(memory)
        ...
        for single_memoryurl in memory_URLsuffix_list:
            singlememory_result2 = H3CR4900.GetInfo(single_memoryurl)
        ...
        #print ('storage',H3CR4900.GetInfo(storage))
        storage_result1 = H3CR4900.GetInfo(storage)
        storage_URLsuffix_list = [x['@odata.id'] for x in storage_result1['Members']]
        for single_storageurl in storage_URLsuffix_list:
            singlestorage_result2 = H3CR4900.GetInfo(single_storageurl)
            if isinstance(singlestorage_result2, dict) and ('error' not in singlestorage_result2.keys()):
                ...
                if disk_count >0: ##有的URL中disk为0，不需要去获取值
                    ...
                    for disk_single in single_disk_URLsuffix_list:
                        single_disk_result1 = H3CR4900.GetInfo(disk_single)
